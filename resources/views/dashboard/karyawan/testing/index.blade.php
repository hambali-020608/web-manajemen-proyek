<x-dashboard-layout>
    ...
    @php
                                $check_done = $testProject->where('is_checked',true)->count();
                                $check_undone = $testProject->where('is_checked',false)->count();
                                 $totalTest = $testProject->count();
                                 $progress_test = $check_done > 0 ? ($totalTest / $check_done) * 100: 0 ;
                                //  $progress_test_undone = $check_done > 0 ? ($totalTest / $check_undone) * 100: 0 ;
                            //  dd($progress_test);
                            @endphp
